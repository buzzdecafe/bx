 ...
 *      g(4); //=> 10
 */
var curryN = _curry2$3(function curryN(length, fn) {
  if (length === 1) {
    return _curry1$3(fn);
  }
...
const coords = index$1.map(toCoords, clicks);

const edge = index$1.map(log('edge'), index$2(isEdge(10), coords));

const grid = index$1.map(log('grid'), index$2(isGrid(10), coords));
