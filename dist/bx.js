...
const cell = (cls, x, y) => rdom_2({ id: pid(x, y), className: 'cell ' + cls }, []);
...
function matrix(width, height) {
  return rdom_2({ className: 'board' }, edgeRow(width, 0).concat(chain(_y => row(width, _y), times(inc, height - 2))).concat(edgeRow(width, height - 1)));
}
...
const ok = index$2(isOk, index$1.stream());
...
const mkElem = s => rdom_2({ className: 'solutionMessage' }, [rdom_3({ className: 'msg' }, [s]), rdom_1({ id: 'ok' }, ['OK'])]);
...
const validate$1 = (pts, gs) => gs.length === pts.length && all(pt => find(whereEq(pt), map(head, gs)), pts);
